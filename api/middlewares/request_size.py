...
async def request_size_limit_middleware(request: Request, call_next: Callable):
    ...
    # Methods without a body never need Content-Length / size enforcement
    if request.method in ("GET", "HEAD", "OPTIONS", "DELETE", "TRACE", "CONNECT"):
        return await call_next(request)
    ...
    if any(request.url.path.startswith(p) for p in UPLOAD_PATH_PREFIXES):
        if content_length is None and "chunked" not in transfer_encoding:
            return JSONResponse(
